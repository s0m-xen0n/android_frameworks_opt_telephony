...
public final class MMTelSSTransport extends Handler {
    ...
    private void requestXcapNetwork(int phoneId) {
        ...
        mNetwork = null;
        if (MMTelSSUtils.isOp01IccCard(phoneId) && (mXcapMobileDataNetworkManager != null)) {
            long subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
            mNetwork = mXcapMobileDataNetworkManager.acquireNetwork(subId);
            if (mNetwork != null) {
                mXcapMobileDataNetworkManager.useAcquiredNetwork(mNetwork,
                        MMTelSSUtils.getXcapRootUri(phoneId));
            }
        }
    }
    ...
    public void setSimservsInitParameters(String xui, String xcapRoot, String intendedId,
        ...
        mSimservs.setXcapRoot(xcapRoot);
    ...
    class MMTelSSTransmitter extends Handler implements Runnable {
        ...
        public void handleGetCF(MMTelSSRequest rr) {
            ...
            String cfNumber = "";
            String CFPhoneNum = "+886936010888";
            ...
            if (setCFAction == CommandsInterface.CF_ACTION_ENABLE || setCFAction == CommandsInterface.CF_ACTION_REGISTRATION) {
                ...
                if (setCFServiceClass == CommandsInterface.SERVICE_CLASS_NONE) {
                    ...
                    if (updateSingleRule && (1 == numExpansion)) {
                        ...
                        Rule newRule = copyOldRuleToNewRuleSetExceptSpecificMedia(r,newRuleSet,setCFServiceClass);
                        if (newRule != null) {
        ...
        /* then copy old rule except voice type into new rule set to present that: BAOC for video call is still enabled */
        public boolean handleCreateNewRuleForExistingCB(SimservType ssType, RuleSet newRuleSet,Rule r,String facility,int lockState,int setCBServiceClass,String RuleID, boolean updateSingleRule, int num_of_expansion) throws XcapException {
            Conditions cond = r.getConditions();
            Actions action = r.getActions();
        ...
        public boolean handleCreateNewRuleForReqCB(SimservType ssType, RuleSet newRuleSet,String facility,int lockState,int setCBServiceClass,String RuleID, boolean updateSingleRule, int num_of_expansion) throws XcapException {
            boolean cbAllow = true;
        ...
        public Rule copyOldRuleToNewRuleSetExceptSpecificMedia(Rule oldRule, RuleSet newRuleSet, int requestedServiceClass) {
            Conditions oldCond = oldRule.getConditions();
            Actions oldAction = oldRule.getActions();
            ...
            if (oldCond != null) {
                ...
                for (int i=0; i < oldMediaList.size(); i++) {
                    if (getMediaType(requestedServiceClass).equals(oldMediaList.get(i)) == false) {
                        newCond.addMedia(oldMediaList.get(i));
                    }
                }
            }
                ...
                for (int it=0; it < num_of_expansion; it++) {
                    ...
                    if (facility.equals(CommandsInterface.CB_FACILITY_BAOC) ||
                                ...
                                if (facility.equals(CommandsInterface.CB_FACILITY_BAOIC) &&
                                    ...
                                    if ((setCBServiceClass != CommandsInterface.SERVICE_CLASS_NONE) || (setCBServiceClass == CommandsInterface.SERVICE_CLASS_NONE && AddRuleForBAOICWithAllMediaType == false)) {
                                        ...
                                        if (setCBServiceClass != CommandsInterface.SERVICE_CLASS_NONE) {
                                            Rule newRule = copyOldRuleToNewRuleSetExceptSpecificMedia(r, newRuleSet, setCBServiceClass);
                                    ...
                                    if ((setCBServiceClass != CommandsInterface.SERVICE_CLASS_NONE) || (setCBServiceClass == CommandsInterface.SERVICE_CLASS_NONE && AddRuleForBAOICxHWithAllMediaType == false)) {
                                        addedNewRule = handleCreateNewRuleForExistingCB(ocb,
                                                ...
                                                mUpdateSingleRule, num_of_expansion);
                                    ...
                                    if ((setCBServiceClass != CommandsInterface.SERVICE_CLASS_NONE) || (setCBServiceClass == CommandsInterface.SERVICE_CLASS_NONE && AddRuleForBAOCWithAllMediaType == false)) {
                                        addedNewRule = handleCreateNewRuleForExistingCB(ocb, newRuleSet,r,facility,lockState,setCBServiceClass, BAOC_RuleID, mUpdateSingleRule, num_of_expansion);
                        ...
                        if (addedNewRule == false) {
                            ...
                            addedNewRule = handleCreateNewRuleForReqCB(ocb, newRuleSet,facility,lockState,setCBServiceClass, newRuleID, mUpdateSingleRule, num_of_expansion);
                        ...
                        if (addedNewRule == false) {
                            ...
                            addedNewRule = handleCreateNewRuleForReqCB(icb, newRuleSet,facility,lockState,setCBServiceClass, newRuleID, mUpdateSingleRule, num_of_expansion);
