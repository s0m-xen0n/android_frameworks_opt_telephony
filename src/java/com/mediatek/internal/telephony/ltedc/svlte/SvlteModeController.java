...
public class SvlteModeController extends Handler {
    ...
    */
    ...
    private SvlteModeController(Context context) {
        ...
        logicLog("Operator Spec:" + mOperatorSpec);
    ...
    private static int[] initCardModes() {
        ...
        for (int i = 0; i < cardModes.length; i++) {
            cardModes[i] = Integer.parseInt(
                SystemProperties.get(SVLTE_PROP, "3,2").split(",")[i]);
        }
        return cardModes;
    }
    ...
    private void switchRadioTechnology(int slotId) {
        ...
        if (sCardModes[slotId] == RADIO_TECH_MODE_SVLTE) {
            ...
            SvlteRatController.SvlteRatMode orginMode = svlteRatController.readSvlteRatMode();
    ...
    private int[] calculateCardMode() {
        ...
        boolean wpSolution2 = CdmaFeatureOptionUtils.isC2KWorldPhoneP2Support();
        if (!wpSolution2) {
            cardModes[0] = (slot0Prefer == RADIO_TECH_MODE_SVLTE) ? RADIO_TECH_MODE_SVLTE
                    : RADIO_TECH_MODE_CSFB;
            cardModes[1] = RADIO_TECH_MODE_CSFB;
            logicLog("[calculateCardMode] >>> WP SOLUTION 1 case.");
            printRadioModes(cardModes);
            return cardModes;
        }
