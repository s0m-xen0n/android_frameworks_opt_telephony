...
public class SvlteRatController extends Handler {
    ...
    private boolean setSvlteRatMode(SvlteRatMode svlteRatMode,
        ...
        if (mRatChangeInRoaming) {
            ...
            mLtePhone.setPreferredNetworkType(
                    RILConstants.NETWORK_MODE_WCDMA_PREF, response);
    ...
    private boolean containsCdmaApp() {
        if (isCtCard(mSlotId)) {
            if (isDualCtCard() && mSlotId == PhoneConstants.SIM_ID_2) {
                return false;
            }
            return true;
        }
    ...
    private void finishSwitchMode(boolean forceFinish, boolean svlteRatChanged,
        ...
        logd("finishSwitchMode, mPendingCardTypeSwitchRecord ="
        ...
        logd("finishSwitchMode, mSvlteRatMode = " + mSvlteRatMode
                + ", mRoamingMode = " + mRoamingMode);
        if (pendingRecord != null) {
            if (pendingRecord.mPendingSetInfo == PENDINGINFO_SET_RAT) {
                ...
                logd("finishSwitchMode, no need to update any rat or roaming info");
            }

            logd("finishSwitchMode, " + pendingRecord);
    ...
    public boolean allowRadioPowerOn(int phoneId) {
        ...
        boolean isGmssOn = isCdma4GSim() && (ratMode == SvlteRatMode.SVLTE_RAT_MODE_4G);
    ...
    public SvlteRatMode readSvlteRatMode() {
        int ratMode = Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.LTE_ON_CDMA_RAT_MODE,
                SvlteRatMode.SVLTE_RAT_MODE_4G.ordinal());
        return SvlteRatMode.values()[ratMode];
    }
