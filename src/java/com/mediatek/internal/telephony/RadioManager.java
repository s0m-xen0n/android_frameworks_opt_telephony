...
public class RadioManager extends Handler  {
    ...
    // When boot under flight mode, then don't set cdma.ril.eboot value,
    // when user enter setting to turnoff airplane mode,
    // don't set cdma.ril.eboot value if mBootUnderAirPlaneFirst = true
    private boolean mBootUnderAirPlaneFirst = false;
    ...
    private RadioManager(Context context , int phoneCount, CommandsInterface[] ci) {
        ...
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()
                && AIRPLANE_MODE_ON == mAirplaneMode) {
            mBootUnderAirPlaneFirst = true;
        }
        mCi = ci;
    ...
    private void setSilentRebootPropertyForAllModem(String isSilentReboot) {
        ...
        switch(config) {
            ...
            case DSDA:
                ...
                if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()
                        && !mBootUnderAirPlaneFirst && mAirplaneMode == AIRPLANE_MODE_OFF) {
                    SystemProperties.set(CDMA_PROPERTY_SILENT_REBOOT_MD, isSilentReboot);
                    mBootUnderAirPlaneFirst = false;
                }
                break;
            case TSTS:
    ...
    public void setModemPower(boolean power, int phoneBitMap) {
        ...
        switch(config) {
            ...
            default:
                ...
                mCi[phoneId].setModemPower(power, null);
                if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                    log("Single SIM mode, for SVLTE need turn on/off MD1");
                    //PhoneFactory.getPhone(SubscriptionManager.LTE_DC_PHONE_ID).setModemPower(power, null);
                    if (mSvlteCi != null) {
                        mSvlteCi.setModemPower(power, null);
                    }
                }
                break;
        }
        ...
        if (phoneId == 0 || phoneId == SubscriptionManager.LTE_DC_PHONE_ID_1) {
            ...
            if (radioTechMode == SvlteModeController.RADIO_TECH_MODE_SVLTE) {
                ...
                if ((iccidCommon == null || iccidCommon.equals("") || iccidCommon.equals("N/A")) &&
                        cardType[0] == UiccController.CARD_TYPE_RUIM) {
                    String iccidC2K = SystemProperties.get(PROPERTY_ICCID_SIM_C2K);
                    if (iccidC2K != null && !iccidC2K.equals("")) {
            ...
            if (radioTechMode == SvlteModeController.RADIO_TECH_MODE_SVLTE) {
                ...
                if ((iccidCommon == null || iccidCommon.equals("") || iccidCommon.equals("N/A")) &&
                        cardType[1] == UiccController.CARD_TYPE_RUIM) {
                    String iccidC2K = SystemProperties.get(PROPERTY_ICCID_SIM_C2K);
                    if (iccidC2K != null && !iccidC2K.equals("")) {
